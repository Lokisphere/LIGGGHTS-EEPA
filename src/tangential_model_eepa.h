...
TANGENTIAL_MODEL(TANGENTIAL_EEPA,eepa,5)
#else
#ifndef TANGENTIAL_MODEL_EEPA_H_
#define TANGENTIAL_MODEL_EEPA_H_
...
{
  template<>
  class TangentialModel<TANGENTIAL_EEPA> : public TangentialModelBase
  {
    double ** coeffFrict;
    int history_offset;
  ...
  public:
    ...
    {
        history_offset = hsetup->add_history_value("shearx", "0");
        hsetup->add_history_value("sheary", "0");
        hsetup->add_history_value("shearz", "0");
    ...
    inline void surfacesIntersect(const SurfacesIntersectData & sidata, ForceData & i_forces, ForceData & j_forces)
    {
        ...
        const double Ft_friction = xmu * fabs(sidata.Fn_contact + sidata.f_min);
