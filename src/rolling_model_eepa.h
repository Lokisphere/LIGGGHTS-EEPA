...
------------------------------------------------------------------------- */
#ifdef ROLLING_MODEL
ROLLING_MODEL(ROLLING_EEPA,eepa,7)
#else
#ifndef ROLLING_MODEL_EEPA_H_
#define ROLLING_MODEL_EEPA_H_
...
namespace ContactModels
{
  ...
  template<>
  class RollingModel<ROLLING_EEPA> : public RollingModelBase
  {
  public:
    ...
    {
      history_offset = hsetup->add_history_value("r_torquex_old", "0");
      hsetup->add_history_value("r_torquey_old", "0");
      hsetup->add_history_value("r_torquez_old", "0");
    ...
    void connectToProperties(PropertyRegistry & registry) {
      ...
      registry.connect("coeffRollFrict", coeffRollFrict,"rolling_model eepa");

      // error checks on coarsegraining
      if(force->cg_active())
        error->cg(FLERR,"rolling model eepa");
    }
  ...
  private:
    ...
    inline void calcRollTorque(double (&r_torque)[3],const SurfacesIntersectData & sidata,double reff,double wr1,double wr2,double wr3) {
      ...
      const double r_torque_max = fabs(sidata.Fn_contact + sidata.f_min)*reff*rmu;
